...
var _resolveBin = require("resolve-bin");
...
function devOrBuild({
  ...
  process.chdir(cwd); // register babel for config files
  ...
  return new Promise((resolve, reject) => {
    const binPath = (0, _resolveBin.sync)('docz');
...
function deploy({
  ...
  return new Promise((resolve, reject) => {
    ...
    assert.ok((0, _fs.existsSync)(distDir), `Please run ${_chalk.default.green(`umi-lib doc build`)} first`);
