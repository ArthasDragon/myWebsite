...
function _buildDoc() {
  _buildDoc = _asyncToGenerator(function* (cwd) {
    const umiLibPath = (0, _path.join)(__dirname, '../bin/umi-library.js');
...
function _doc() {
  _doc = _asyncToGenerator(function* (name) {
    ...
    const targetDist = (0, _path.join)(cwd, '.docz/dist');
...
_asyncToGenerator(function* () {
  ...
  yield page.goto(`http://localhost:${servers['normal'].port}/#/button`, {
    waitUntil: 'networkidle2'
  });
...
_asyncToGenerator(function* () {
  ...
  const favicon = yield page.evaluate(() => document.querySelectorAll('link')[1].href);
